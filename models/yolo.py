...
class TSCODE_Detect(nn.Module):
    ...
    def __init__(self, nc=80, anchors=(), ch=(), inplace=True):  # detection layer
        ...
        self.m_cls = nn.ModuleList(nn.Sequential(Conv(sum(ch[id:id + 2]), ch[id], 3),
                                                 nn.Conv2d(ch[id], self.na * self.nc * 4, 1)) for id in
                                   range(1, len(ch) - 1))  # cls conv
        self.m_reg_conf = nn.ModuleList(nn.Sequential(*[Conv(ch[id], ch[id], 3) for i in range(1)]) for id in
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cfg', type=str, default='../BTYOLO.yaml', help='model.yaml')
